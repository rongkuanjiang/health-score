...
(() => {
  ...
  const profiles = [['blank','Manual entry'],['example','Worked example · 80 points + WBC flag'],['low','Low hs-CRP · 100 points + WBC flag'],['high','Above 10 mg/L · withheld'],['bound','hs-CRP <1 · plateau points'],['uncertain-bound','hs-CRP <2 · withheld'],['cbc','Blood-cell results only'],['standard','Standard CRP · context only'],['unknown','Unknown collection context'],['pregnant','Pregnancy · withheld'],['younger','Age 16 · observations only']];
  ...
  root.innerHTML = `<div class="section-head domain-intro"><div><h2>Inflammation markers</h2><p>One conditional hs-CRP marker score, with separate laboratory observations.</p></div></div>
    ...
    <section class="panel"><details><summary>Blood-cell results and ESR · optional context</summary><p>These results do not contribute points. Use each result’s own date and laboratory flags. Differential percentages remain percentages.</p>${fields.slice(1).join('')}</details></section>
    <button id="infl-calculate" type="submit">Calculate hs-CRP marker score</button><p id="infl-error" role="alert"></p></form>
    ...
    <section class="panel"><details class="education"><summary>Understand these markers and points</summary><p>This experimental score describes your hs-CRP result at collection. It does not measure all inflammation or diagnose its cause. A low result does not exclude inflammatory disease. There is no combined inflammation score.</p><h4>hs-CRP</h4><p>The point curve is a provisional design, not an endorsed clinical scale. Standard CRP and tests of unknown sensitivity remain context only. Above-range results and uncertain bounds can withhold points. A bounded result is never shown as an exact concentration.</p><div id="infl-curve"></div><h4>Blood cells and ESR</h4><p>These provide separate context. White-cell findings can have different causes; this prototype retains the laboratory interpretation without converting it into inflammation points. Missing tests are not zero scores and do not imply that additional testing is required.</p><p>Read more: <a href="https://medlineplus.gov/lab-tests/c-reactive-protein-crp-test/" target="_blank" rel="noopener noreferrer">CRP testing</a> · <a href="https://medlineplus.gov/lab-tests/white-blood-count-wbc/" target="_blank" rel="noopener noreferrer">White blood cell counts</a></p></details></section>`;
  ...
  function render(r) {
    ...
    out.append(node('h4','hs-CRP marker score'),node('strong',c.display_score === null ? '—' : `${c.display_score} / 100`));
    ...
    if (c.score === null) out.append(node('p','A suitable hs-CRP result and collection context are needed for this score. Available blood-cell results are shown separately.'));
    for (const reason of c.reasons) out.append(node('p',reasonText(reason),'notice'));
    notices(out,r.notices); notices(out,c.notices);
    out.append(node('h4','Coverage'),node('p',`hs-CRP supplied: ${r.coverage.hs_crp_present ? 'yes' : 'no'}; usable for points: ${r.coverage.hs_crp_usable ? 'yes' : 'no'}. Context markers: ${r.coverage.context_markers_present.map(human).join(', ') || 'none'}. Limited marker coverage; no comprehensive inflammation total.`));
