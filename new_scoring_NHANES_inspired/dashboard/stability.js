...
(() => {
  ...
  function render(r){
    ...
    const out=el('results');out.replaceChildren();out.append(node('h3',r.domain_score===null?'Score unavailable':`${r.display_score} / 100`),node('p',`${r.coverage.scored}/${r.coverage.required} scoring markers usable. Sodium 50% + potassium 50%. Provisional, not clinically validated.`));if(r.review_required)out.append(node('p','Abnormal result or laboratory flag present. Review individual results even when the total is high.','notice'));for(const reason of r.score_reasons)out.append(node('p',explain(reason),'notice'));el('state').textContent=headlines[r.panel_status]||human(r.panel_status);
